...
export default function LeadershipClient({ videos, showHeader = true }: LeadershipClientProps) {
  ...
  const handleVideoClick = (videoId: string) => {
    ...
    router.push(`/leadership/${videoId}`);
  };
    ...
    <>
      ...
      {loadingVideo && (
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          {/* Gradient Background */}
          <div className="absolute inset-0 bg-gradient-to-br from-purple-900/95 via-blue-900/95 to-black/95 backdrop-blur-sm" />
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-6 mb-16">
            {videos.map((video) => (
              <Card key={video.id} className={`h-full bg-gradient-to-br from-gray-800/50 to-gray-900/50 border-gray-700/50 hover:border-purple-500/30 transition-all duration-300 hover:shadow-xl hover:shadow-purple-500/10 ${
                loadingVideo === video.id ? 'opacity-75 scale-95' : ''
              }`}>
                  ...
                  <div className="p-6 pt-0">
                    <div className="flex gap-3">
                      <button
                        ...
                        className="flex-1 inline-flex items-center justify-center px-4 py-2 bg-gradient-to-r from-blue-500 to-purple-600 text-white hover:from-blue-600 hover:to-purple-700 focus:ring-blue-500 shadow-lg hover:shadow-xl font-medium transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-gray-900 text-sm rounded-lg"
                      >
