...
interface DeveloperCreateTaskModalProps {
  onClose: () => void;
  onTaskCreated: () => void;
}
...
export const DeveloperCreateTaskModal = ({ onClose, onTaskCreated }: DeveloperCreateTaskModalProps) => {
  ...
  const [isMondayRelated, setIsMondayRelated] = useState<boolean>(false);
  ...
  // Update time estimates when complexity changes
  useEffect(() => {
    if (complexity === "high") {
      setEstimatedTime(40);
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      const createResponse = await fetch("/api/developer/tasks/create", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          ...
          is_monday_related: isMondayRelated,
          ...videoFields
        }),
      });
  ...
  return (
    <div className="fixed inset-0 bg-black/50 flex items-start justify-center z-50 p-4 overflow-y-auto">
      <div className="relative bg-gray-800 rounded-xl p-6 max-w-2xl w-full my-8">
        ...
        <div className="bg-blue-900/50 border border-blue-500/50 rounded-xl p-4 mb-6">
          <div className="flex items-center">
            <IconPlus className="h-5 w-5 text-blue-400 mr-2" />
            <p className="text-blue-200 text-sm">
              <strong>Note:</strong> You will be automatically assigned to this task upon creation.
            </p>
          </div>
        </div>
        ...
        {/* Monday.com Related Checkbox */}
        <div className="mb-6 bg-gray-900/50 p-4 rounded-lg border border-gray-700">
          <label className="flex items-center space-x-3">
            <input
              type="checkbox"
              checked={isMondayRelated}
              onChange={(e) => setIsMondayRelated(e.target.checked)}
              className="rounded bg-gray-700 border-gray-600 text-indigo-500 focus:ring-indigo-500 focus:ring-offset-gray-800"
            />
            <div>
              <span className="text-sm font-medium text-gray-300">Monday.com Related Task</span>
              <p className="text-xs text-gray-400">Check this if the task involves Monday.com integration or related work</p>
            </div>
          </label>
        </div>
